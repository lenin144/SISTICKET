<?php
	...
	if (empty($_POST['ticked_id'])) {
	    ...
	    $users=mysqli_query($con,"select * from user where id=$asigned_id");
	    $rw_users=mysqli_fetch_array($users);
	    $name_users=$rw_users['name']." ".$rw_users['lastname'];
	    $email_agente=$rw_users['email'];
	    // $text="<b>Asignado a:</b> $name_users";
	    $text="Ticket Asignado a $name_users";
...
$url_base=$rw_url_base['val'];
		...
		asigne_ticket($email_user,$email_password,$from_name,$Host,$Port,$url_base,  $email_agente,$fullname_supervisor,$ticket_number,$asunt,$comment,$id,$nombre_categoria,$nombre_area,$fecha,$name_priority);
		...
		$sql="UPDATE tickets SET status_id=2, asigned_id=\"$asigned_id\", tipo_requerimiento=$tipo_requerimiento, area_id=$id_area, asignado=NOW() WHERE id=$id";
		$query_update = mysqli_query($con,$sql);
			if ($query_update){
				$messages[] = "La asignación ah sido ingresada satisfactoriamente.";
				//sleep(1);
				print("<script>window.location='./?view=registrados'</script>");
