<?php	
	...
	if (empty($_POST['nombre'])) {
        ...
        $sql = "UPDATE `datos` SET `nombre` = \"$nombre\", `apellido` = \"$apellido\" , `departamento` = \"$departamento\", `sucursal` = \"$sucursal\", `categoria` = \"$categoria\", `edad` =\"$edad\", `marca` =\"$marca\", `modelo` =\"$modelo\", `equipo` =\"$equipo\", `serial` =\"$serial\", `so` =\"$so\", `procesador` =\"$procesador\", `memoria` =\"$memoria\", `estado` =\"$estado\"  WHERE `datos`.`id` = \"$iddato\"";
							...
							if($_FILES["imagefile"]["name"] != ""){
								...
								$allowed_extension = array("doc","docx","xls","xlsx","pdf","zip","jpg","png","jpeg","DOC","DOCX","XLS","XLSX","ZIP","JPG","PNG","JPEG","PDF");
								   if(!in_array($file_name[1], $allowed_extension)){ 
									...
									$sql = "UPDATE `datos` SET `adjunto1` = \"$img_insert\" WHERE `datos`.`id` = \"$iddato\"";
									$query_new_insert = mysqli_query($con,$sql);
									if($query_new_insert){ 
										// $messages[] = "success";
									}else{
							...
							if($_FILES["imagefile2"]["name"] != ""){
								...
								$allowed_extension = array("doc","docx","xls","xlsx","pdf","zip","jpg","png","jpeg","DOC","DOCX","XLS","XLSX","ZIP","JPG","PNG","JPEG","PDF");
								   if(!in_array($file_name[1], $allowed_extension)){ 
									...
									$sql = "UPDATE `datos` SET `adjunto2` = \"$img_insert\" WHERE `datos`.`id` = \"$iddato\"";
									$query_new_insert = mysqli_query($con,$sql);
									if($query_new_insert){ 
										// $messages[] = "success";
									}else{
							...
							if($_FILES["imagefile3"]["name"] != ""){
								...
								$allowed_extension = array("doc","docx","xls","xlsx","pdf","zip","jpg","png","jpeg","DOC","DOCX","XLS","XLSX","ZIP","JPG","PNG","JPEG","PDF");
								   if(!in_array($file_name[1], $allowed_extension)){ 
									...
									$sql = "UPDATE `datos` SET `adjunto3` = \"$img_insert\" WHERE `datos`.`id` = \"$iddato\"";
									$query_new_insert = mysqli_query($con,$sql);
									if($query_new_insert){ 
										// $messages[] = "success";
									}else{
