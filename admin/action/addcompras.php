<?php	
	...
	if (empty($_POST['numcom'])) {
        ...
        $sql = "INSERT INTO `compras` (`id`, `numcom`, `tipo`, `numero`, `razon`, `fecha`, `bienservicio`, `total`) VALUES (NULL, \"$numcom\", \"$tipo\", \"$numero\", \"$razon\", \"$fecha\", \"$bienservicio\", \"$total\")";
		$query_new_insert = mysqli_query($con,$sql);
			if ($query_new_insert){
				$messages[] = "El dato ha sido ingresado satisfactoriamente.";
				//sleep(1);

                $last=mysqli_query($con,"SELECT LAST_INSERT_ID(`id`) AS 'last' FROM `compras` order by id desc limit 0,1 ");
				        ...
				        foreach($_FILES['imagefile']['name'] as $name => $value)  {  
							...
							$allowed_extension = array("doc","docx","xls","xlsx","pdf","zip","jpg","png","jpeg","DOC","DOCX","XLS","XLSX","ZIP","JPG","PNG","JPEG","PDF");
				           	if(!in_array($file_name[1], $allowed_extension)){ 
				            	...
				            	$sql = "UPDATE `compras` SET `adjunto".$cont."` = \"$img_insert\" WHERE `compras`.`id` = \"$iddato\"";
				            	$query_new_insert = mysqli_query($con,$sql);
				            	if($query_new_insert){ 
