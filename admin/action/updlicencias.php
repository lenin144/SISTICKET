<?php	
	...
	if (empty($_POST['equipo'])) {
        ...
        $sql = "UPDATE `licencias` SET `equipo` = \"$equipo\", `responsable` = \"$responsable\" , `departamento` = \"$departamento\", `sucursal` = \"$sucursal\", `programa` = \"$programa\", `licencia` = \"$licencia\", `correo` = \"$correo\", `fechareg` = \"$fechareg\", `fechaven` = \"$fechaven\", `estado` = \"$estado\", `observacion` = \"$observacion\", `nompro` = \"$nompro\", `numpro` = \"$numpro\" WHERE `licencias`.`id` = \"$iddato\"";
							...
							if($_FILES["imagefile"]["name"] != ""){
								...
								$allowed_extension = array("doc","docx","xls","xlsx","pdf","zip","jpg","png","jpeg","DOC","DOCX","XLS","XLSX","ZIP","JPG","PNG","JPEG","PDF");
								   if(!in_array($file_name[1], $allowed_extension)){ 
									...
									$sql = "UPDATE `licencias` SET `adjunto1` = \"$img_insert\" WHERE `licencias`.`id` = \"$iddato\"";
									$query_new_insert = mysqli_query($con,$sql);
									if($query_new_insert){ 
										// $messages[] = "success";
									}else{
							...
							if($_FILES["imagefile2"]["name"] != ""){
								...
								$allowed_extension = array("doc","docx","xls","xlsx","pdf","zip","jpg","png","jpeg","DOC","DOCX","XLS","XLSX","ZIP","JPG","PNG","JPEG","PDF");
								   if(!in_array($file_name[1], $allowed_extension)){ 
									...
									$sql = "UPDATE `licencias` SET `adjunto2` = \"$img_insert\" WHERE `licencias`.`id` = \"$iddato\"";
									$query_new_insert = mysqli_query($con,$sql);
									if($query_new_insert){ 
										// $messages[] = "success";
									}else{
							...
							if($_FILES["imagefile3"]["name"] != ""){
								...
								$allowed_extension = array("doc","docx","xls","xlsx","pdf","zip","jpg","png","jpeg","DOC","DOCX","XLS","XLSX","ZIP","JPG","PNG","JPEG","PDF");
								   if(!in_array($file_name[1], $allowed_extension)){ 
									...
									$sql = "UPDATE `licencias` SET `adjunto3` = \"$img_insert\" WHERE `licencias`.`id` = \"$iddato\"";
									$query_new_insert = mysqli_query($con,$sql);
									if($query_new_insert){ 
										// $messages[] = "success";
									}else{
