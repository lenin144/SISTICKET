<?php 
    ...
    $idsurvey=intval($_GET['idsurvey']);
    $id=intval($_GET['id']);
    if (isset($_GET['id']) && !empty($_GET['id'])){
        $id=$_GET["id"];
    } else {
        header("Location: ?view=encuestor_questions&idsurvey=$idsurvey");  
    }
    ...
    $sql=mysqli_query($con, "select * from surveyquestions where idQuestion=$id");
                        ...
                        <form role="form" method="post" name="upd" id="upd">
                            <div class="box-body">
                                <div class="form-group">
                                    ...
                                    <textarea required type="text" name="description" class="form-control" id="description" placeholder="Descripción..." rows="10"><?php echo $Description ?></textarea>
                                </div>
...
$( "#upd" ).submit(function( event ) {
 ...
 var parametros = $(this).serialize();
     $.ajax({
            type: "POST",
            url: "action/updencuestorquestion.php",
            data: parametros,
