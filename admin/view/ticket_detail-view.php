...
if (isset($_GET['id']) && !empty($_GET['id'])){
    ...
    $rows01=mysqli_fetch_array($requerimientos);
       $name_require=$rows01['name'];  
    ...
    $rows03=mysqli_fetch_array($status);
       $name_status=$rows03['name'];

    // $clientes_sql=mysqli_query($con,"select * from user where id=$client_id and is_client=1");
    $clientes_sql=mysqli_query($con,"select * from user where id=$client_id");
    $rw_client=mysqli_fetch_array($clientes_sql);
    // $name_client=$rw_client['business'];
    $name_client=$rw_client['name']." ".$rw_client['lastname'];
    ...
    <div class="content-wrapper">
        ...
        <div class="row">
            <div class="col-md-12">
                ...
                <?php if($is_admin!=1){?>
                <a class="btn btn-default pull-right" href="javascript:history.back()"><i class="glyphicon glyphicon-arrow-left"></i> Regresar</a>
                <?php } ?>
            </div>
        </div>
        ...
        <section class="content">
            ...
            <?php// if(($is_admin!=1 and $soy_supervisor!=1) and $status_id==3){?>
            <div style="text-align: center;">
                <?php if($status_id==3 and $status_id!=5){?>
                    <a class="btn btn-success" href="?view=atender_ticket&id=<?php echo $id_ticket ?>"><i class="fa fa-check"></i> Finalizar Ticket</a>
                <?php } ?>
                <?php if($status_id==2 or $status_id==3){?>
                    <a class="btn btn-danger" href="?view=incidencias&id=<?php echo $id_ticket ?>"><i class="fa fa-ban"></i> Agregar Bitacora</a>
                <?php } ?>
            </div>
            <?php// } ?>
        </section><!-- /.content -->
